...
>
    ...
    <span class="flex shrink-0 items-center gap-1 text-slate-400">
        <span x-show="value && allowEmpty && !disabled" @click.stop="clear()" class="rounded px-1 text-xs hover:bg-slate-100 hover:text-slate-700" title="Clear">&times;</span>
...
>
    ...
    <ul class="max-h-56 overflow-y-auto py-1 text-sm">
        <template x-if="allowEmpty">
            <li>
                <button type="button" class="flex w-full px-3 py-2 text-left text-slate-500 hover:bg-amber-50" @click="clear()" x-text="emptyLabel"></button>
            </li>
        </template>
