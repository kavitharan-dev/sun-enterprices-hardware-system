    ...
    <form method="POST" action="{{ isset($worker) ? route('construction.workers.update', $worker) : route('construction.workers.store') }}" class="max-w-2xl space-y-4 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div class="grid gap-4 sm:grid-cols-2">
            ...
            <div>
                ...
                <x-text-input id="daily_rate" name="daily_rate" type="number" step="0.01" min="0" class="mt-1 block w-full" :value="old('daily_rate', isset($worker) ? $worker->daily_rate : '0')" />
            </div>
            <div>
                <x-input-label for="weekly_salary" value="Weekly salary (Rs.)" />
                <x-text-input id="weekly_salary" name="weekly_salary" type="number" step="0.01" min="0" class="mt-1 block w-full" :value="old('weekly_salary', isset($worker) ? $worker->weekly_salary : '0')" />
